// google ai summary lol
function getFitFontSize(text, containerWidth, containerHeight, currentSize) {
    // Create a hidden canvas or element to measure text
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
    ctx.font = `${currentSize}px sans-serif`;

    const textWidth = ctx.measureText(text).width;
    const textHeight = currentSize; // Approximation for single line
    ...
    const res = Math.floor(currentSize * bestScale);
    return res;
}
// https://fdossena.com/?p=html5cool/radprog/i.frag
...
function RadialProgress(container, cfg) {
    ...
    this.animationSpeed =
        cfg.animationSpeed == undefined
            ...
            : cfg.animationSpeed > 0
              ...
              : 1;
    this.noPercentage =
        cfg.noPercentage == undefined ? false : cfg.noPercentage;
    ...
    this.draw = function (f) {
        ...
        if (document.getElementById("code") != null) {
            this.text.style.fontSize = `${getFitFontSize(
        ...
        if (this.indeterminate) {
            this.indetA =
                (this.indetA + 0.07 * this.animationSpeed) % (2 * Math.PI);
            this.indetB =
                (this.indetB + 0.14 * this.animationSpeed) % (2 * Math.PI);
            ...
            if (this.spin && !this.noAnimations)
                this.rot =
                    (this.rot + 0.07 * this.animationSpeed) % (2 * Math.PI);
            c.arc(
                centerX,
                centerY,
                radius,
